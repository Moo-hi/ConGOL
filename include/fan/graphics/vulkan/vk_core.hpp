		...
		namespace gpu_memory {
			...
			struct buffer_object {
				...
				void map_data(VkDeviceSize size, VkDeviceSize offset = 0) {
					...
					memcpy(data, m_instance.data(), size);
				...
				void edit_data(uint32_t i) {
					...
					fan::vk::gpu_memory::copy_buffer(*buffer->m_device, *pool, *graphics_queue, staging->m_buffer_object, buffer->m_buffer_object, bufferSize, 0, sizeof(object_type) * i);
				}
